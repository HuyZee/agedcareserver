...
router.patch('/:id', async (req, res) => {
    ...
    try {
        const updatedMedication = await prisma.medication.update({
            where: { medication_id: parseInt(id) },
            data: updateData
        });
        res.json(updatedMedication);
    } catch (error) {
        console.error('Error partially updating medication:', error);
        if (error.code === 'P2025') { // Check if the medication does not exist
